...
# SIX TYPES: match mb-{any} class via regex
types = [
    (r'(Unoxidized\s+[^\s<>]+\s+Fresh\s+[^\s<>]+\s+Vegetal)',
     r'Unoxidized \u00b7 Fresh \u00b7 Vegetal',
     ...
     'Green'),
    (r'(Minimally\s+Processed\s+[^\s<>]+\s+Delicate\s+[^\s<>]+\s+Sweet)',
     r'Minimally Processed \u00b7 Delicate \u00b7 Sweet',
     ...
     'White'),
    (r'(Lightly\s+Oxidized\s+[^\s<>]+\s+Mellow\s+[^\s<>]+\s+Rare)',
     r'Lightly Oxidized \u00b7 Mellow \u00b7 Rare',
     ...
     'Yellow'),
    (r'(Partially\s+Oxidized\s+[^\s<>]+\s+Floral\s+[^\s<>]+\s+Roasted)',
     r'Partially Oxidized \u00b7 Floral to Roasted',
     ...
     'Oolong'),
    (r'(Fully\s+Oxidized\s+[^\s<>]+\s+Malty\s+[^\s<>]+\s+Robust)',
     r'Fully Oxidized \u00b7 Malty \u00b7 Robust',
     ...
     'Black'),
    (r'(Post-Fermented\s+[^\s<>]+\s+Earthy\s+[^\s<>]+\s+Aged)',
     r'Post-Fermented \u00b7 Earthy \u00b7 Aged',
...
for pattern, new_label, desc, label in types:
    full = r'<p class="text-sm text-zinc-500 mb-\d">' + pattern + r'</p>'
    m = re.search(full, content)
    if m:
        ...
        # Fallback: just replace the inner content
        m2 = re.search(pattern, content)
        if m2:
            print(label + ': found content but not p-tag: ' + repr(m2.group(0)[:40]))
        else:
...
# GONGFU
m = re.search(r'(A traditional brewing method using small clay pots.*?formally\.)', content, re.DOTALL)
if m:
